	...
	public double calculate(MVideo mtr1,MVideo mtr2 ){
		...
		for(int i = 1; i< dist_matrix.length; i++){
			for(int j = 1; j < dist_matrix[0].length; j++){	
				...
				double pointX1 = Math.abs(mtr1.getFov()[j].getCoord().x-mtr2.getFov()[i].getCoord().x);
				double pointX2 = Math.abs(mtr1.getFov()[j-1].getCoord().x-mtr2.getFov()[i-1].getCoord().x);
				parallel = Math.min(pointX1, pointX2);
				// get prependicular distance 
				double pointY1 =Math.abs(mtr1.getFov()[j].getCoord().y-mtr2.getFov()[i].getCoord().y);
				double pointY2 = Math.abs(mtr1.getFov()[j-1].getCoord().y-mtr2.getFov()[i-1].getCoord().y);
